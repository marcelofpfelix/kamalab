...
kama_listen=${KAMA_LISTEN:-127.0.0.2}
# local kamailio repo
...
# Write compose env to .env so variables survive sudo / watch / any
# invocation context. docker compose auto-loads .env from the project
# directory. .env is gitignored.
cat > .env <<EOF
KAMA_IMG=$kama_img
KAMA_CFG=$kama_cfg
...
docker compose pull
docker compose down
docker compose -p kam up --force-recreate -V -d app # --watch
